<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <meta http-equiv="X-UA-Compatible" content="ie=edge">
  <title>Document</title>

  <link rel="stylesheet" href="{{asset('dash/css/modal.css')}}">
  <script defer src="{{asset('dash/scripts/modal.js')}}"></script>
</head>
<body>

  <div class="contendor">
    <h2>Texto de prueba <button type="button" id="modal-open">Open</button></h2>

    <p>
      Lorem ipsum dolor sit amet, consectetur adipisicing elit. Distinctio nihil minima porro vel, 
      fuga nam ducimus provident, ipsum sapiente temporibus facilis nesciunt! Quasi harum odio dolore, 
      dolores consequuntur numquam quia.
      Lorem ipsum dolor sit amet, consectetur adipisicing elit. Distinctio nihil minima porro vel, 
      fuga nam ducimus provident, ipsum sapiente temporibus facilis nesciunt! Quasi harum odio dolore, 
      dolores consequuntur numquam quia.
      Lorem ipsum dolor sit amet, consectetur adipisicing elit. Distinctio nihil minima porro vel, 
      fuga nam ducimus provident, ipsum sapiente temporibus facilis nesciunt! Quasi harum odio dolore, 
      dolores consequuntur numquam quia.
    </p>
  
  
    <p>
      Lorem ipsum dolor sit amet, consectetur adipisicing elit. Distinctio nihil minima porro vel, 
      fuga nam ducimus provident, ipsum sapiente temporibus facilis nesciunt! Quasi harum odio dolore, 
      dolores consequuntur numquam quia.
      Lorem ipsum dolor sit amet, consectetur adipisicing elit. Distinctio nihil minima porro vel, 
      fuga nam ducimus provident, ipsum sapiente temporibus facilis nesciunt! Quasi harum odio dolore, 
      dolores consequuntur numquam quia.
      Lorem ipsum dolor sit amet, consectetur adipisicing elit. Distinctio nihil minima porro vel, 
      fuga nam ducimus provident, ipsum sapiente temporibus facilis nesciunt! Quasi harum odio dolore, 
      dolores consequuntur numquam quia.
    </p>
  
    <p>
      Lorem ipsum dolor sit amet, consectetur adipisicing elit. Distinctio nihil minima porro vel, 
      fuga nam ducimus provident, ipsum sapiente temporibus facilis nesciunt! Quasi harum odio dolore, 
      dolores consequuntur numquam quia.
      Lorem ipsum dolor sit amet, consectetur adipisicing elit. Distinctio nihil minima porro vel, 
      fuga nam ducimus provident, ipsum sapiente temporibus facilis nesciunt! Quasi harum odio dolore, 
      dolores consequuntur numquam quia.
      Lorem ipsum dolor sit amet, consectetur adipisicing elit. Distinctio nihil minima porro vel, 
      fuga nam ducimus provident, ipsum sapiente temporibus facilis nesciunt! Quasi harum odio dolore, 
      dolores consequuntur numquam quia.
    </p>
  
    <p>
      Lorem ipsum dolor sit amet, consectetur adipisicing elit. Distinctio nihil minima porro vel, 
      fuga nam ducimus provident, ipsum sapiente temporibus facilis nesciunt! Quasi harum odio dolore, 
      dolores consequuntur numquam quia.
      Lorem ipsum dolor sit amet, consectetur adipisicing elit. Distinctio nihil minima porro vel, 
      fuga nam ducimus provident, ipsum sapiente temporibus facilis nesciunt! Quasi harum odio dolore, 
      dolores consequuntur numquam quia.
      Lorem ipsum dolor sit amet, consectetur adipisicing elit. Distinctio nihil minima porro vel, 
      fuga nam ducimus provident, ipsum sapiente temporibus facilis nesciunt! Quasi harum odio dolore, 
      dolores consequuntur numquam quia.
    </p>
  
    <p>
      Lorem ipsum dolor sit amet, consectetur adipisicing elit. Distinctio nihil minima porro vel, 
      fuga nam ducimus provident, ipsum sapiente temporibus facilis nesciunt! Quasi harum odio dolore, 
      dolores consequuntur numquam quia.
      Lorem ipsum dolor sit amet, consectetur adipisicing elit. Distinctio nihil minima porro vel, 
      fuga nam ducimus provident, ipsum sapiente temporibus facilis nesciunt! Quasi harum odio dolore, 
      dolores consequuntur numquam quia.
      Lorem ipsum dolor sit amet, consectetur adipisicing elit. Distinctio nihil minima porro vel, 
      fuga nam ducimus provident, ipsum sapiente temporibus facilis nesciunt! Quasi harum odio dolore, 
      dolores consequuntur numquam quia.
    </p>
  </div>




  <section class="modal" id="modal">

    <div class="modal--content">
      <div class="modal--header">
        <button class="modal--header--btn-close" id="modal-close">
          <img src="{{asset('dash/assets/icon-close.svg')}}" alt="icon close">
        </button>
      </div>
  
      <div class="modal--body">
        
        <section class="modal--body-limit">
          <h2 class="modal--body--title">Seleccionar Cliente</h2>
          <form class="modal--body--search">
            <input class="modal--body--search-input" type="text" placeholder="Escriba una zona">
            <button class="modal--body--search-icon" type="submit">
              <img src="{{asset('dash/assets/icon-search.svg')}}" alt="icon close">
            </button>
          </form>

          <table class="modal--body--table">
            <tr>
              <th>Numero</th>
              <th>Usuario</th>
              <th>CI</th>
              <th>Placa 1</th>
              <th>Placa 2</th>
              <th>Placa 3</th>
            </tr>
    
            <tr>
              <td>1</td>
              <td>Juan Perez</td>
              <td>36652445</td>
              <td>1852PDH</td>
              <td>1852PDH</td>
              <td>1852PDH</td>
            </tr>
    
            <tr>
              <td>2</td>
              <td>Juan Perez</td>
              <td>36652445</td>
              <td>1852PDH</td>
              <td>1852PDH</td>
              <td>1852PDH</td>
            </tr>
    
            <tr>
              <td>3</td>
              <td>Juan Perez</td>
              <td>36652445</td>
              <td>1852PDH</td>
              <td>1852PDH</td>
              <td>1852PDH</td>
            </tr>
    
            <tr>
              <td>4</td>
              <td>Juan Perez</td>
              <td>36652445</td>
              <td>1852PDH</td>
              <td>1852PDH</td>
              <td>1852PDH</td>
            </tr>
    
            <tr>
              <td>5</td>
              <td>Juan Perez</td>
              <td>36652445</td>
              <td>1852PDH</td>
              <td>1852PDH</td>
              <td>1852PDH</td>
            </tr>

            <tr>
              <td>6</td>
              <td>Juan Perez</td>
              <td>36652445</td>
              <td>1852PDH</td>
              <td>1852PDH</td>
              <td>1852PDH</td>
            </tr>
          </table>
        </section>

      </div>
    </div>

  </section>


</body>
</html>